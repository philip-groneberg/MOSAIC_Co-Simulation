...
"""
Script to create mosaic vtypes based on carla blueprints.
"""
...
def generate_vtype(vehicle):
    """Generates mosaic vtype specification for a given carla vehicle.

        :param vehicle: carla actor (carla.Actor)
        :return: mosaic vtype specifications
    """
    ...
    if type_id not in CARLA_BLUEPRINTS_SPECS:
        ...
        if number_of_wheels == 2:
            logging.warning(
                '''type id %s not mapped to any mosaic vtype.
