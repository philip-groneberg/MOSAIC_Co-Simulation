...
"""
Script to generate mosaic nets based on opendrive files. Internally, it uses netconvert to generate
...
class MosaicTopology(object):
    """
    This object holds the topology of a mosaic net. Internally, the information is structured as
    follows:
        ...
        - odr2mosaic_ids: {
            (odr_road_id, odr_lane_id): [(mosaic_edge_id, mosaic_lane_id), ...], ...}
    """
    def __init__(self, topology, paths, odr2mosaic_ids):
        # Contains only standard roads.
        self._topology = topology
        # Contaions only roads that belong to a junction.
        self._paths = paths
        # Mapped ids between mosaic and opendrive.
        self._odr2mosaic_ids = odr2mosaic_ids

    # http://mosaic.sourceforge.net/userdoc/Networks/Import/OpenDRIVE.html#dealing_with_lane_sections
    def get_mosaic_id(self, odr_road_id, odr_lane_id, s=0):
        """
        Returns the pair (mosaic_edge_id, mosaic_lane index) corresponding to the provided odr pair. The
        argument 's' allows selecting the better mosaic edge when it has been split into different
        edges due to different odr lane sections.
        """
        if (odr_road_id, odr_lane_id) not in self._odr2mosaic_ids:
            return None

        mosaic_ids = list(self._odr2mosaic_ids[(odr_road_id, odr_lane_id)])
        ...
        # s coordinate of the provided landmark.
            ...
            # Ensures that all the related mosaic edges belongs to the same opendrive road but to
            # different lane sections.
            assert set([edge.split('.', 1)[0] for edge, lane_index in mosaic_ids]) == 1

            s_coords = [float(edge.split('.', 1)[1]) for edge, lane_index in mosaic_ids]

            s_coords, mosaic_ids = zip(*sorted(zip(s_coords, mosaic_ids)))
            index = bisect.bisect_left(s_coords, s, lo=1) - 1
            return mosaic_ids[index]
    ...
    def get_successors(self, mosaic_edge_id, mosaic_lane_index):
        """
        Returns the successors (standard roads) of the provided pair (mosaic_edge_id, mosaic_lane_index)
        """
        if self.is_junction(mosaic_edge_id, mosaic_lane_index):
            return []

        return list(self._topology.get((mosaic_edge_id, mosaic_lane_index), set()))
...
def build_topology(mosaic_net):
    """
    Builds mosaic topology.
    """
    # --------------------------
    # OpenDrive->Mosaic mapped ids
    # --------------------------
    # Only takes into account standard roads.
    #
    #   odr2mosaic_ids = {(odr_road_id, odr_lane_id) : [(mosaic_edge_id, mosaic_lane_index), ...], ...}
    odr2mosaic_ids = {}
    for edge in mosaic_net.getEdges():
        for lane in edge.getLanes():
            if lane.getParam('origId') is None:
                raise RuntimeError(
                    'Mosaic lane {} does not have "origId" parameter. Make sure that the --output.original-names parameter is active when running netconvert.'
                    .format(lane.getID()))

            if len(lane.getParam('origId').split()) > 1:
                logging.warning('[Building topology] Mosaic net contains joined opendrive roads.')

            for odr_id in lane.getParam('origId').split():
                odr_road_id, odr_lane_id = odr_id.split('_')
                if (odr_road_id, int(odr_lane_id)) not in odr2mosaic_ids:
                    odr2mosaic_ids[(odr_road_id, int(odr_lane_id))] = set()
                odr2mosaic_ids[(odr_road_id, int(odr_lane_id))].add((edge.getID(), lane.getIndex()))

    # -----------
    # Connections
    # -----------
    #
    #   topology -- {(mosaic_road_id, mosaic_lane_index): [(mosaic_road_id, mosaic_lane_index), ...], ...}
    #   paths    -- {(odr_road_id, odr_lane_id): [
    #                   ((mosaic_edge_id, mosaic_lane_index), (mosaic_edge_id, mosaic_lane_index))
    #               ]}
    topology = {}
    paths = {}

    for from_edge in mosaic_net.getEdges():
        for to_edge in mosaic_net.getEdges():
            ...
            for connection in connections:
                ...
                if conn_odr_ids is not None:
                    if len(conn_odr_ids.split()) > 1:
                        logging.warning(
                            '[Building topology] Mosaic net contains joined opendrive paths.')
    ...
    return MosaicTopology(topology, paths, odr2mosaic_ids)
...
class MosaicTrafficLight(object):
    """
    MosaicTrafficLight holds all the necessary data to define a traffic light in mosaic:
    ...
    def generate_tl_id(from_edge, to_edge):
        """
        Generates mosaic traffic light id based on the junction connectivity.
        """
    ...
    def generate_default_program(tl):
        """
        Generates a default program for the given mosaic traffic light
        """
        ...
        for road in set(incoming_roads):
            ...
            tl.add_phase(MosaicTrafficLight.DEFAULT_DURATION_GREEN_PHASE, ''.join(phase_green))
            tl.add_phase(MosaicTrafficLight.DEFAULT_DURATION_YELLOW_PHASE, ''.join(phase_yellow))
            tl.add_phase(MosaicTrafficLight.DEFAULT_DURATION_RED_PHASE, ''.join(phase_red))
    ...
    def add_phase(self, duration, state, min_dur=-1, max_dur=-1, next_phase=None, name=''):
        ...
        """
        self.phases.append(
            MosaicTrafficLight.Phase(duration, state, min_dur, max_dur, next_phase, name))
    ...
    def add_landmark(self,
        ...
        connection = MosaicTrafficLight.Connection(tlid, from_road, to_road, from_lane, to_lane,
                                                 link_index)
...
def _netconvert_carla_impl(xodr_file, output, tmpdir, guess_tls=False):
    ...
    tmp_mosaic_net = os.path.join(tmpdir, basename + '.net.xml')

    try:
        basedir = os.path.dirname(os.path.realpath(__file__))
        result = subprocess.call(['netconvert',
            '--opendrive', xodr_file,
            '--output-file', tmp_mosaic_net,
            ...
            # Necessary to link odr and mosaic ids.
    ...
    # Mosaic net
    # --------
    mosaic_net = mosaiclib.net.readNet(tmp_mosaic_net)
    mosaic_topology = build_topology(mosaic_net)
    ...
    for landmark in landmarks:
        ...
        for from_lane, to_lane in landmark.get_lane_validities():
            for lane_id in range(from_lane, to_lane + 1):
                ...
                if wp.is_junction and mosaic_topology.is_junction(road_id, lane_id):
                    tlid = str(wp.get_junction().id)
                    if tlid not in tls:
                        tls[tlid] = MosaicTrafficLight(tlid)
                    tl = tls[tlid]

                    if guess_tls:
                        for from_edge, from_lane in mosaic_topology.get_incoming(road_id, lane_id):
                            successors = mosaic_topology.get_successors(from_edge, from_lane)
                            for to_edge, to_lane in successors:
                                tl.add_landmark(landmark.id, tl.id, from_edge, to_edge, from_lane,
                                                to_lane)

                    else:
                        connections = mosaic_topology.get_path_connectivity(road_id, lane_id)
                ...
                elif not wp.is_junction and not mosaic_topology.is_junction(road_id, lane_id):
                    from_edge, from_lane = mosaic_topology.get_mosaic_id(road_id, lane_id, landmark.s)

                    for to_edge, to_lane in mosaic_topology.get_successors(from_edge, from_lane):
                        tlid = MosaicTrafficLight.generate_tl_id(from_edge, to_edge)
                        if tlid not in tls:
                            tls[tlid] = MosaicTrafficLight(tlid)
                        tl = tls[tlid]
    ...
    tree = ET.parse(tmp_mosaic_net, parser)
    root = tree.getroot()

    for tl in tls.values():
        MosaicTrafficLight.generate_default_program(tl)
        edges_tags = tree.xpath('//edge')
        if not edges_tags:
            raise RuntimeError('No edges found in mosaic net.')
